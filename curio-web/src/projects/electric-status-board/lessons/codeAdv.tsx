...
export const LESSON_STEPS_ADVANCED: Record<number, { phrase: string; advanced?: boolean; optional?: boolean; steps: any[] }> = {
  // =========================================================
  // LESSON 1
  // =========================================================
  1: {
...
**Together, these libraries allow the Arduino to communicate with the OLED and render text/graphics (and optionally read time from a clock module).**`,
...

      {
        ...
        blankExplanations: {
          ...
          HEIGHT2:
            "Use the HEIGHT constant you defined above so the constructor matches your screen’s height.",
        },
        blankDifficulties: {
          WIDTH: "easy",
          HEIGHT: "easy",
...
- That same HEIGHT constant must be reused inside the \`display(...)\` constructor.`,
            imageGridAfterCode: null,
            descAfterImage: null,
...
Example: If your PREV button is connected to digital pin 3, then PREV would be 3. Fill the rest based on your wiring.`,
...

      {
        ...
        blankExplanations: {
          ...
          CLEAR:
            "Clear the display buffer so you start with a blank screen.",
          SETTEXTSIZE:
            "Function that sets how large the text will appear.",
          SETTEXTSIZE2:
            "Pick a text size number (1–5). Bigger number = bigger text.",
          SETTEXTCOLOR:
            "Function that sets the text color mode.",
          SETTEXTCOLOR2:
            "Pick a text color mode constant (normal white text is most common).",
          SETCURSOR:
            "Function that moves the cursor to an (x,y) pixel position.",
          XCOORD:
            "Choose an x-position on the screen where the text should start.",
          YCOORD:
            "Choose a y-position on the screen where the text should start.",
          DISPLAY:
            "Push the buffer to the physical screen so it updates.",
          NEXT:
...
void setup() {
  ...
  __BLANK[CLEAR]__;       // clear display buffer
  __BLANK[SETTEXTSIZE]__(__BLANK[SETTEXTSIZE2]__);      // text size
  __BLANK[SETTEXTCOLOR]__(__BLANK[SETTEXTCOLOR2]__);    // text color mode
  __BLANK[SETCURSOR]__(__BLANK[XCOORD]__, __BLANK[YCOORD]__);      // cursor location
  __BLANK[DISPLAY]__;     // update the OLED
...
export default function CodeAdvLesson({
  slug,
  lessonSlug,
}: {
  slug: string;
